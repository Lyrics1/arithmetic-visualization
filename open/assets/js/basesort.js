...
$('.kaishi').attr('disabled',true);//开始标签禁止点击GO 后可点击
var TIME = 0//标记是否是第一次点击随机数按钮，防止第一次，timer没有定义
auto();
$('.click-random').click(function(){
	auto();
	/*$('.kaishi').attr('disabled',true);
	console.log(TIME)
	if(TIME){clearInterval(timer); 	 $('.showexe').eq(0).empty();hideCheck();
	//$('.result').eq(0).empty();
}
            ...
            //，在出现createElement();
	            	...
	            	for(var f=0;f<10;f++){
	           		...
	           		if(bucket == f){
	           			ten[bucket]+=1;          			
	           			temp =ten[bucket];
	           		    console.log("length:"+lastArray.length);
...
function clearResult(){
	// var showexe = document.getElementsByClassName('showexe')[0];
	// var blockend = document.getElementsByClassName('blockend');
	// var blockendLen = blockend.length;
	setTimeout(function(){
		$('.showexe').eq(0).empty();

		// for(var end=0;end<blockendLen;end++){
