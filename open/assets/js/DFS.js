window.onload=function(){
	...
	$(".click-random").click(function(){
		...
		LINE= create();
		clearInterval(timer);
		$('.code-2 li').css("background","transparent");
	...
	function change(){
		...
		speed =2500- $('.click-speed-1').val()*200;
		console.log(speed,$('.click-speed-1').val())
	}
...
var T= ["A","B","C","D","E","F","G","H","I","J","K","L"];
 		$('circle').eq(i).css("fill","rgb(242, 182, 75)");
 		$('#explain').append(T[i]+"  ");
 		setTimeout(function(){
 			$('circle').eq(i).css("fill","rgba(226, 12, 12, 0.66)");
 		}, 350)
 ...
 function changeDemo(i,speed){
 	setTimeout(function(){$('#l li').eq(i).css("background","rgba(241, 199, 73, 0.83)");
 }, speed)
